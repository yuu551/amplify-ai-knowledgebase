...
export function request(ctx) {
  ...
  const streamingResponseMutation = {
    name: 'createAssistantResponseStreamChatHttp',
    inputTypeName: 'CreateConversationMessageChatHttpAssistantStreamingInput',
    selectionSet,
  };
  ...
  const dataTools = [{"name":"searchDocumentation","description":"Performs a similarity search over the documentation","inputSchema":{"json":{"type":"object","properties":{"input":{"type":"string","description":"A UTF-8 character sequence."}},"required":["input"]}},"graphqlRequestInputDescriptor":{"selectionSet":"response sourceDocuments error","propertyTypes":{"input":"String!"},"queryName":"queryKnowledgeBaseHttp"}}];
  const toolsConfiguration = { dataTools, clientTools };

  const messageHistoryQuery = {
    getQueryName: 'getConversationMessageChatHttp',
    getQueryInputTypeName: 'ID',
    listQueryName: 'listConversationMessageChatHttps',
    listQueryInputTypeName: 'ModelConversationMessageChatHttpFilterInput',
...
export function response(ctx) {
  ...
  const response = {
    __typename: 'ConversationMessageChatHttp',
